...
```{r, eval=FALSE}
Fit1 <- add_criterion(model1, "loo", moment_match = TRUE)
...
Models are in order of 4,5,2,3 using loo_compare. But differences are small. This also is sensitive to small changes in the priors.
...
extract_IS <- function(m, x, group, re_formula=NULL, modelID="id"){
  if (is.na(group))
    d <- m$data |> slice(1) 
    else d <- m$data |> group_by(.data[[group]]) |> slice(1) |> ungroup()
  d[[x]] <- 0
  ...
  if (is.na(group))
    result |>  mutate(group="global") |> select(all_of(group), term,rvar, modelID) 
    else result |> select(all_of(group), term, rvar, modelID) |> rename(group=group)
}
